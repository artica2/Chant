...
public class RARE : MonoBehaviour {
	...
	public string ExportClip(string fileName, AudioClip myClip, Action<AudioClip, string> callBackFunction = null, GameObject popUp = null){
		if (popUp != null) {
			popUp.SetActive (true);
		}
		//create the filepath.
		string filepath = Application.persistentDataPath + "/recordings/";
		...
		FileStream fileStream;
		fileStream = new FileStream(filepath + fileName + ".wav", FileMode.Create);
		byte emptyByte = new byte();
		for(int i = 0; i<headerSize; i++) { //preparing the header
			fileStream.WriteByte(emptyByte);
		}
